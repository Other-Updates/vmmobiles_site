...
class ModelAccountCustomer extends Model {
	...
	public function addCustomer1($data) {
		if (isset($data['customer_group_id']) && is_array($this->config->get('config_customer_group_display')) && in_array($data['customer_group_id'], $this->config->get('config_customer_group_display'))) {
			$customer_group_id = $data['customer_group_id'];
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	public function check_customer_name($id){
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` WHERE `customer_id` = '" . $id . "'");
		if(empty($query->row['firstname']) && empty($query->row['lastname']) && empty($query->row['lastname'])){
			$data="Show Account Edit";
			return $data;
		}
	}
}